...
Simple rule-based strategies with:
- Take profit percentage (sell when price increases X%)
- Stop loss percentage (sell when price decreases X%)
- Buy dip percentage (buy when price decreases X%)
"""
...
class StrategyService:
    ...
    def create_strategy(
        ...
        Args:
            ...
            take_profit_percent: Percentage to sell when price increases (positive, e.g., 5 = sell at +5%)
            stop_loss_percent: Percentage to sell when price decreases (positive, e.g., 3 = sell at -3%)
            buy_dip_percent: Optional percentage to buy when price decreases (positive, e.g., 5 = buy at -5%)
        ...
        try:
            ...
            if take_profit_percent <= 0:
                return {
                    'success': False,
                    'error': 'take_profit_percent must be positive (e.g., 5 for +5%)'
                }
            
            if stop_loss_percent <= 0:
                return {
                    'success': False,
                    'error': 'stop_loss_percent must be positive (e.g., 3 for -3%)'
                }
            
            if buy_dip_percent is not None and buy_dip_percent <= 0:
                return {
                    'success': False,
                    'error': 'buy_dip_percent must be positive (e.g., 5 for -5%)'
                }
            ...
            strategy = {
                ...
                'rules': {
                    ...
                    'buy_dip_percent': float(buy_dip_percent) if buy_dip_percent else None
                },
                'is_active': is_active,
            ...
            logger.info(f"Strategy created: {token} on {exchange['nome']} (TP: +{take_profit_percent}%, SL: -{stop_loss_percent}%)")
